...
async function callClaude({ system, user, content, maxTokens = 1000 }) {
...
export async function parseResumePdf({ base64, mediaType = 'application/pdf' }) {
...
{ "resume": ${RESUME_SCHEMA.replace(/\n/g, ' ')}, "missing": [] }`;
  ...
  return callClaude({ system, content, maxTokens: 8000 });
}
...
export async function fillResumeGaps({ resume, answers }) {
